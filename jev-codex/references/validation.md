...
## Native lifecycle hook development — 2026-09-22

Codex CLI 0.153.4 exposes PreCompact, PostCompact and SessionStart hooks. 33 offline tests pass, including six lifecycle handler tests. A real Jev 1.13.0 call with synthetic transcript data preserved constraints/failure/pending work and restored them once after simulated PostCompact; approximately 0.88s API latency. These tests do not establish native compaction integration. After explicit authorization for scoped transcript egress, the three exact definitions were trusted through the normal Codex /hooks interface. A separate App Server test then observed native contextCompaction completion, successful PreCompact/PostCompact/SessionStart hook notifications, and JEV_COMPACT_RESTORE delivered as context. The next model response correctly recalled COBALT-FERN, the publication prohibition and pending export retry, and reported the restored notes present. The test conversation was archived. The hook state was consumed once, fallback=false, with one real Jev call (~0.81s). This validates native integration in Codex CLI/App Server 0.153.4; it does not measure retention improvement over native compaction alone or prove already-running desktop sessions reload changed settings.
