...
export async function main(args=process.argv.slice(2)){
 ...
 try{
  ...
  if(!key&&!(mode==='context'&&!input.action&&Array.isArray(input.items)&&input.items.reduce((n,i)=>n+(i.text?.length||0),0)<(input.minChars??8000)))throw new Stop('missing_api_key');
  result=await({context:input.action?compactMode:contextMode,search:searchMode,review:reviewMode,supervisor:supervisorMode,browser:browserMode}[mode])(input,jev,{output});
